...
local astroDir = path.getabsolute("..")
local naclToolchain = ""
...
--			premake.gcc.llvm = true
	...
	configuration { "vs*" }
		includedirs { path.join(astroDir, "include/compat/msvc") }
		defines {
			"WIN32",
			"_WIN32",
			"_HAS_EXCEPTIONS=0",
	...
	configuration { "vs2008" }
		includedirs { path.join(astroDir, "include/compat/msvc/pre1600") }
	...
	configuration { "mingw-*" }
		defines { "WIN32" }
		includedirs { path.join(astroDir, "include/compat/mingw") }
		buildoptions {
			"-Wunused-value",
			"-fdata-sections",
			"-ffunction-sections",
...
--			"ubsan",
	...
	configuration { "freebsd" }
		...
		includedirs {
			path.join(astroDir, "include/compat/freebsd"),
		}
	...
	configuration { "nacl or nacl-arm or pnacl" }
		...
		includedirs {
			...
			path.join(astroDir, "include/compat/nacl"),
		}
...
--		includedirs { path.join(astroDir, "include/compat/qnx") }
		buildoptions {
			"-Wno-psabi", -- note: the mangling of 'va_list' has changed in GCC 4.4.0
			"-Wunused-value",
			"-Wundef",
